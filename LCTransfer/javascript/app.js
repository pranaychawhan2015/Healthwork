...
app.get('/patients/:email', async (req, res) => {
  try {
    ...
    const contract = network.getContract('healthwork');
      const result = await contract.evaluateTransaction('queryPatient', req.params.email, '', signatureJSON, message);
      console.log(result.toString());
...
app.post('/patients', async (req, res) => {
  ...
  try {
    ...
    await RegisterAdmins(wallet);
    console.log(3);
    let userExists = await wallet.get(req.body.email);
    ...
    console.log(userCCP);
    userExists = await wallet.get(req.body.email);
    const gateway = new Gateway();
    ...
    await transaction.submit(req.body.key, req.body.name, req.body.age, req.body.specialization, req.body.disease, req.body.email,req.body.adhar, caName, '', signatureJSON, message)
    console.log(6);
...
app.put('/patients', async (req, res) => {
  try {
    ...
    let priv_keys = []
    network.getChannel().getEndorsers().forEach(endorser=>{
      if(element.name.startsWith("peer0.org1.example.com") )
      {
          endorsers.push(element);
          priv_keys.push(ccp.peers['peer0.org1.example.com'].privateKey.pem)
      }
      if(element.name.startsWith("peer1.org1.example.com")){
          endorsers.push(element);
          priv_keys.push(ccp.peers['peer0.org1.example.com'].privateKey.pem)
      }
      if(element.name.startsWith("peer2.org1.example.com") )
      {
          endorsers.push(element);
          priv_keys.push(ccp.peers['peer2.org1.example.com'].privateKey.pem)
      }
      if(element.name.startsWith("peer3.org1.example.com")){
          endorsers.push(element);
          priv_keys.push(ccp.peers['peer3.org1.example.com'].privateKey.pem)
      }
      if(element.name.startsWith("peer0.org2.example.com") )
      {
          endorsers.push(element);
          priv_keys.push(ccp2.peers['peer0.org2.example.com'].privateKey.pem)
      }
      if(element.name.startsWith("peer1.org2.example.com")){
          endorsers.push(element);
          priv_keys.push(ccp2.peers['peer1.org2.example.com'].privateKey.pem)
      }
      if(element.name.startsWith("peer2.org2.example.com") )
      {
          endorsers.push(element);
          priv_keys.push(ccp2.peers['peer2.org2.example.com'].privateKey.pem)
      }
      if(element.name.startsWith("peer3.org2.example.com")){
          endorsers.push(element);
          priv_keys.push(ccp2.peers['peer3.org2.example.com'].privateKey.pem)
      }
    })

    let privData = {priv_peer:priv_keys}
    ...
    console.log(req.body.operation);
         ...
         discovery.sign(new IdentityContext(userContext, network.getChannel().client));
           ...
           let build_proposal_request = {
            args: ['dischargeReport', req.body.patient.Email,req.body.key, data, signatureJSON, message]
            //fcn: 'initLedger'
          };
           ...
           if (req.body.operation === "testsampleReport")
           {
              ...
              args: ['testsampleReport', req.body.patient.Email,req.body.key, data, signatureJSON, message]
              //fcn: 'initLedger'
              };      
           }
