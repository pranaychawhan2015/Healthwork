...
func selectPeersForLayout(endorsersByGroups map[string][]*Peer, layout map[string]int, f Filter) (Endorsers, bool) {
	...
	for grp, count := range layout {
		...
		fmt.Println("Filter", f, "group", grp, "count", count, "endorsers By Group", endorsersByGroups)
		...
		fmt.Println("endorsers of grp1", endorsersOfGrp)

		index := 0
		for key, value := range endorsersOfGrp {
			newArray := strings.Split(string(value.Identity), value.MSPID)
			newArray2 := strings.Split(newArray[1], "-----BEGIN CERTIFICATE-----")
			newArray3 := "-----BEGIN CERTIFICATE-----" + newArray2[1]
			pem2, _ := pem.Decode([]byte(newArray3))
			cert, err2 := x509.ParseCertificate(pem2.Bytes)
			if err2 != nil {
				panic(err2)
			}
			fmt.Println("peer address from cert", cert.DNSNames)
			if value.MSPID == "Org1MSP" && cert.DNSNames[0] == "peer0.org1.example.com" {
				index = key
			}
			if value.MSPID == "Org2MSP" && cert.DNSNames[0] == "peer0.org2.example.com" {
				index = key
			}
		}
		//endorsersOfGrp = endorsersOfGrp[:count]
		endorsersOfGrp = endorsersOfGrp[index]

		fmt.Println("endorsers from new ", endorsersOfGrp)
