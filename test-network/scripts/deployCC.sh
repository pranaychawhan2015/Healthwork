...
queryCommitted() {
  ...
  PEER=$2
  setGlobals $ORG $PEER
  EXPECTED_RESULT="Version: ${CC_VERSION}, Sequence: ${CC_SEQUENCE}, Endorsement Plugin: , Validation Plugin: vscc"
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    set -x
    peer lifecycle chaincode querycommitted -o orderer1.example.com:7050 orderer2.example.com:7055 orderer3.example.com:7056 --ordererTLSHostnameOverride orderer1.example.com orderer2.example.com orderer3.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} >&log.txt
    res=$?
    { set +x; } 2>/dev/null
    test $res -eq 0 && VALUE=$(cat log.txt | grep -o '^Version: '$CC_VERSION', Sequence: [0-9]*, Endorsement Plugin: , Validation Plugin: vscc')
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 localhost:7055 localhost:7056 --ordererTLSHostnameOverride orderer1.example.com orderer2.example.com orderer3.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
